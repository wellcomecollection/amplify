...
cors = CORS(app, resources={r"/visionStage1": {"origins": "http://localhost:4204"},
                            r"/post_vision": {"origins": "http://localhost:4204"}})
...
def get_visionStage1():
    ...
    google_vision_api_response = google_vision_api.detect_text(
        image, from_path=False)
    print('Translating text...')
    google_translate_api_response = google_translate_api.translate_text(
        google_vision_api_response)
    try:
        print('Analyzing text...')
        google_natural_language_api_response = google_natural_language_api.analyze_entities(
            google_translate_api_response['translatedText'])
        author, title, date, publisher, publisher_place, meta_data = google_natural_language_api.retrieve_entities(
            google_natural_language_api_response)
    except:
        author, title, date, publisher, publisher_place, meta_data = None, None, None, None, None, None

    print('Running Open Search on WorldCat...')
    worldcat_search_api_open_search_response = worldcat_search_api.open_search(
        google_vision_api_response)
    try:
        print('Retrieving record identifiers from WorldCat Open Search response...')
        record_identifier_dict = parse_message.get_record_identifiers(
            worldcat_search_api_open_search_response)
    except:
        record_identifier_dict = [
            {'record_identifier': 'No results', 'title': 'No results'}]
...
def library_hub_search():
    ...
    try:
        ...
        library_hub_api_response = library_hub_api.search_record(
            author=author, title=title, publisher=publisher)
        print('Showing results...')
        library_hub_api_response = library_hub_api.show_results(
...
def abim_search():
    ...
    response = abim_search_api.search_record(
        author=author, title=title, publisher=publisher, date=date)

    results = abim_search_api.parse_response(response)

    print('Running Open Search on WorldCat...')
    worldcat_search_api_open_search_response = worldcat_search_api.open_search(
        abim_search_api.format_search_string(title))
...
def get_visionStage2():
    print('Extracting text...')
    google_vision_api_response = request.get_json()[
        'google_vision_api_response']
    print('Translating text...')
    google_translate_api_response = google_translate_api.translate_text(
        google_vision_api_response)
